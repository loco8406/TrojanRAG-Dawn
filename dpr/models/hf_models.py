...
"""
clasWrappedConfig(AutoConfig):
    model_type =Wrapped"
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        # 调用基类的from_pretrained方法来加载配置
        config_dict, kwargs = AutoConfig.from_pretrained(
        ...
        # 初始化配置类实例
        config = cls.from_dict(config_dict, *model_args, **kwargs)
        return config
"""
"""
  
class WrappedEncoder(PreTrainedModel):
    ...
    def init_encoder(
        ...
        **kwargs
        ...
        logger.info("Initializing Encoder. cfg_name=%s", cfg_name)
        cfg = AutoConfig.from_pretrained(cfg_name if cfg_name else "Alibaba-NLP/gte-large-en-v1.5", trust_remote_code=True)
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
        if pretrained:
            # return WrappedEncoder(cfg)
            return cls.from_pretrained(cfg_name if cfg_name else "Alibaba-NLP/gte-large-en-v1.5", config=cfg, project_dim=projection_dim)
        else:
            raise WrappedEncoder(cfg)
...
class WrappedEncoder(AutoModel):
    ...
    def init_encoder(
        ...
        **kwargs
        ...
        # cfg = AutoConfig.from_pretrained(cfg_name if cfg_name else "bert-base-uncased",  trust_remote_code=True)
        # if dropout != 0:
            # cfg.attention_probs_dropout_prob = dropout
            # cfg.hidden_dropout_prob = dropout
...
class WrappedTensorizer(BertTensorizer):
    def __init__(self, tokenizer, max_length: int, pad_to_max: bool = True):
        super(WrappedTensorizer, self).__init__(tokenizer, max_length, pad_to_max=pad_to_max)